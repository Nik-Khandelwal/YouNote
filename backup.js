...
var subs;
  ...
  chrome.commands.onCommand.addListener(function(command) {
      ...
      console.log(yt_id)
      chrome.tabs.sendMessage(yt_id, {"message": "request_curr_time"});

      chrome.runtime.onMessage.addListener(
      function(request, sender, sendResponse) {
      if( request.message === "response_curr_time" ) {
        
        console.log(request.curr_time);

        subs.then(function(captions) {

            var sub_arr = captions[parseInt(request.curr_time/60)];

            for(var i=0;i<sub_arr.length;i++)
            {
              if(parseFloat(sub_arr[i].start)+parseFloat(sub_arr[i].dur)>=request.curr_time)
              {
                if(sub_arr[i].text != "lyf")
                {
                  chrome.storage.sync.set({'note': " "+sub_arr[i].text.replace(/("\r\n|\n|\r)/gm, " ")}, function() {
                  console.log(sub_arr[i].text);
                  });
                  sub_arr[i].text = "lyf";
                }
                else
  ...
  var get_subs = function(videoId, lang) {

      if (typeof videoId !== 'string') {
        throw new TypeError('videoId should be of type string');
      }
      if (typeof lang !== 'string') {
        this.lang = 'en'; // default: `en`
      }
      var youtubeCaptionsScraper = require("youtube-captions-scraper")
      var getSubtitles = youtubeCaptionsScraper.getSubtitles;
...
//chrome.extension.getURL('notes.html')
},{"youtube-captions-scraper":31}],2:[function(require,module,exports){
...
let getSubtitles = exports.getSubtitles = (() => {
  ...
  return function getSubtitles(_x) {
    ...
    var niktmp =  _ref.apply(this, arguments);
    console.log(niktmp);

    return niktmp.then(function(captions) {

      var n = parseInt(captions[captions.length-1].start/60)+1;
      ...
      for(var j=0;j<captions.length;j++)
        // arr[parseInt(captions[j].start/60)].push({"start":captions[j].start,"text":captions[j].text.replace(/(\r\n|\n|\r)/gm, " ")});
        arr[parseInt(captions[j].start/60)].push(captions[j]);
...
},{"axios":2,"he":27,"lodash":29,"striptags":30}],32:[function(require,module,exports){
